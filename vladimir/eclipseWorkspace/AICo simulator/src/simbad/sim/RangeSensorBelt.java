...
public class RangeSensorBelt extends PickSensor {
	 ...
	 *            range.
	 */
	public RangeSensorBelt(Vector3d[] positions, Vector3d[] directions,
			int type, int flags) {
	...
	private void initialize(float radius, float maxRange, int nbsensors,
			int type, int flags) {
		this.flags = flags;
		...
		protected void paintComponent(Graphics g) {
			...
			for (int i = 0; i < nbSensors; i++) {
				...
				if (type == TYPE_BUMPER)
					...
					g.drawString(
							"[" + i + "] " + format.format(measurements[i]), x,
							y);
			}
			int cx, cy;
			cx = cy = IMAGE_SIZEX / 4;
